...
//////////////////////////////////////////////////////////////////
	...
	logic	[1+1+FLOAT_E_DW-1:0]		e_initial_res_mul_temp;		// 1+ care for ovf or 2'compl, result of the sum of exponents
	logic	[1+1+FLOAT_E_DW-1:0]		e_initial_extra_neg_temp;	// 1+ care for ovf or 2'compl, result of the sum of exponents
	logic	[1+1+FLOAT_E_DW-1:0]		e_initial_res_mul;			// 1+ care for ovf or 2'compl, result of the sum of exponents
	logic	[2*(1+FLOAT_F_DW)-1:0]		f_initial_dsp_res_mul;		// f = f_op1 * f_op2 (twice the width)
	logic	[(1+1+FLOAT_F_DW+3)-1:0]	f_initial_res_mul;			// f = f_op1 * f_op2
...
//////////////////////////////////////////////////////////////////
	...
	always_comb
	begin
		e_initial_res_mul_temp		= extE_op1_i + extE_op2_i - FLOAT_E_BIAS - nlz_op1_i - nlz_op2_i;
		e_initial_extra_neg_temp	= FLOAT_E_BIAS + nlz_op1_i + nlz_op2_i - extE_op1_i - extE_op2_i;
	...
	//
		...
		if (e_initial_res_mul_temp[(1+1+FLOAT_E_DW)-1])
		begin
			if(e_initial_extra_neg_temp > 1+FLOAT_F_DW+3)
			begin
		...
		else if (e_initial_res_mul_temp >= FLOAT_E_MAX)
		begin
			f_initial_dsp_res_mul		= '0;
			e_initial_res_mul			= -1;
		end
		...
		f_initial_res_mul		= f_initial_dsp_res_mul[(2*(1+FLOAT_F_DW)-1) -:(1+1+FLOAT_F_DW+3/*G,R,S*/)];
		...
		stickyBit				= |f_initial_dsp_res_mul[0+: (2*(1+FLOAT_F_DW)-1) -(1+1+FLOAT_F_DW+3) ];

		f_initial_res_mul[0]	= f_initial_res_mul[0] | stickyBit;

		if (f_initial_res_mul[1/*extra bit*/+(1/*hidden 1*/+FLOAT_F_DW+3/*LSB rnd bits*/)-1] == 1)
			// op1 + op2 = f_res = {-> 1 <-, {x/*hidden*/,23{x}}, xxx}
			if (e_initial_res_mul + 1 == FLOAT_E_MAX)
			...
			begin // DO POST-NORM (single right shift) since e+1 < 0xff
				...
				f_res_postNorm		= {1'b0,f_initial_res_mul [1+1+FLOAT_F_DW+3-1:1]}; // shift f right by 1. NOTE: trash the sticky bit by 1 rsh of f
			end
		...
		begin
			...
			f_res_postNorm			= {f_initial_res_mul [1+1+FLOAT_F_DW+3-1:2], f_initial_res_mul[1] | f_initial_res_mul[0], 1'b0 /*old useless sticky*/ };
			e_res_postNorm			= e_initial_res_mul;
		end
		else if (f_initial_res_mul[1/*extra bit*/+(1/*hidden 1*/+FLOAT_F_DW+3/*LSB rnd bits*/)-2] == 1)
		begin
			isUnderflow_postNorm	= '1; 
			e_res_postNorm			= '0; 
			f_res_postNorm			= {1'b0, f_initial_res_mul[1+1+FLOAT_F_DW+3-1:1]};
		end
		else
		begin
			isUnderflow_postNorm	= '1;
